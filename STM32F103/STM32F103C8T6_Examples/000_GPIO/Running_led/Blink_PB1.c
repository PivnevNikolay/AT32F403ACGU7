...
void PORTB_2_INIT(void) {
  ...
  MODIFY_REG(GPIOB->CRL, GPIO_CRL_MODE1, Output_mode_max_speed_50_MHz << GPIO_CRL_MODE1_Pos);  //Настройка GPIOC порта 13 на выход со максимальной скоростью в 50 MHz
  MODIFY_REG(GPIOB->CRL, GPIO_CRL_CNF1, General_purpose_output_push_pull << GPIO_CRL_CNF1_Pos); //Настройка GPIOC порта 13 на выход в режиме Push-Pull
  //GPIO_ResetOutputPin(GPIOB, GPIO_PIN_1);
  WRITE_REG(GPIOB->BRR, GPIO_PIN_1);
  delay_simple(5000);
}
...
int main(void) {
  ...
  while (1) {
 // WRITE_REG(GPIOB->ODR, READ_REG(GPIOB->ODR) ^ GPIO_PIN_1);
 // DWT_Delay_ms(500);
 //--------------------------------------------------------------------
